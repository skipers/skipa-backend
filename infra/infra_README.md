...
이 디렉토리는 SKIPA 백엔드 배포 및 운영에 필요한 Kubernetes, Argo CD, Datastore 관련 설정을 관리합니다.
...
백엔드 Service는 `ClusterIP` 타입으로 생성합니다.  
...
백엔드처럼 상태를 직접 저장하지 않는 애플리케이션 서버는 Deployment로 실행합니다.  
...
PostgreSQL과 Redis는 StatefulSet 기반으로 실행합니다.

```bash
kubectl apply -f infra/k8s/datastore/postgres-service.yml
kubectl apply -f infra/k8s/datastore/postgres-statefulset.yml

kubectl apply -f infra/k8s/datastore/redis-service.yml
kubectl apply -f infra/k8s/datastore/redis-statefulset.yml
```
...
백엔드 manifest는 Argo CD가 관리합니다.  
수동으로 확인하거나 임시 적용할 때는 아래 명령어를 사용할 수 있습니다.
...
```bash
kubectl logs -f deployment/skipa-backend
```

## Argo CD 설정

공용 Argo CD namespace는 아래와 같습니다.

```text
skala-argocd
```

SKIPA 백엔드 Argo CD Application 이름은 아래와 같습니다.
...
```text
infra/argocd/skipa-backend-application.yml
```

Argo CD Application 생성:

```bash
kubectl apply -f infra/argocd/skipa-backend-application.yml
```
...
kubectl describe application team8-skipa-backend -n skala-argocd
```

## GitHub Actions + Argo CD 배포 흐름

백엔드는 GitHub Actions와 Argo CD를 함께 사용해 배포합니다.
...
→ Docker image 빌드
→ Harbor push
→ backend-deployment.yml의 image tag 자동 수정
→ 변경된 manifest가 main에 commit
→ Argo CD가 Git 변경 감지
→ EKS에 자동 sync
→ RollingUpdate로 백엔드 Pod 교체
```

GitHub Actions는 이미지를 빌드하고 Harbor에 push한 뒤, Kubernetes manifest의 image tag를 업데이트합니다.  
Argo CD는 Git에 반영된 manifest를 기준으로 EKS 클러스터 상태를 동기화합니다.

## GitHub Secrets

GitHub Actions에서 Harbor에 이미지를 push하기 위해 아래 repository secret이 필요합니다.
...
- Harbor 비밀번호, AWS Access Key, JWT Secret은 코드에 직접 작성하지 않습니다.
...
- DB/Redis 같은 Stateful 리소스는 백엔드 배포와 분리해서 관리합니다.
